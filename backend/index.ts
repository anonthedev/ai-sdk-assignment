...
async function generateVideo({
  ...
  console.log(`[${style.toUpperCase()}] Starting video generation for prompt: ${prompt}`);
  ...
  console.log(`[${style.toUpperCase()}] Narration generated: ${narration.substring(0, 100)}...`);
  ...
  while (!op.done) {
    pollCount++;
    console.log(`[${style.toUpperCase()}] Waiting for video generation... (poll #${pollCount})`);
  ...
  if (!videos || videos.length === 0) {
    console.error(`[${style.toUpperCase()}] Video generation failed - no videos returned`);
  ...
  for (let i = 0; i < videos.length; i++) {
    ...
    if (uri) {
      console.log(`[${style.toUpperCase()}] Video ${i + 1} URI generated: ${uri}`);
      ...
      try {
        ...
        console.log(`[${style.toUpperCase()}] Video ${i + 1} saved to: ${filePath}`);
      } catch (error) {
        console.error(`[${style.toUpperCase()}] Failed to download video ${i + 1}:`, error);
  ...
  console.log(`[${style.toUpperCase()}] Video generation complete - ${filePaths.length} videos saved`);
  return { filePaths };
}

const PromptSchema = z.object({ prompt: z.string() });

const createHypeAgent = () => new Agent<{ prompt: string }>({
  name: "Hype Video Agent",
  instructions: "You create fast-paced, high-energy hype videos using your tools.",
  tools: [
    tool({
      name: "generate_hype_video",
      description: "Generate a high-energy hype video from a user prompt.",
      ...
      execute: async ({ prompt }) => {
        ...
        return await generateVideo({
          prompt,
          style: "hype",
          narrationPrompt: `Write a high-energy hype script with action verbs and excitement for: ${prompt}`,
          visualPrompt: `Energetic visuals for: ${prompt}, with bold colors and motion.`,
        });
      },
    }),
  ],
    ...
    tool({
      ...
      execute: async ({ prompt }) => {
        ...
        return await generateVideo({
          prompt,
          style: "ad",
          narrationPrompt: `Write a professional, product-focused ad script for: ${prompt}`,
          visualPrompt: `Clean, commercial visuals for: ${prompt}`,
        });
      },
    }),
  ],
...
const createCinematicAgent = () => new Agent<{ prompt: string }>({
  ...
  tools: [
    tool({
      ...
      description: "Generate a cinematic emotional video.",
      parameters: PromptSchema,
      execute: async ({ prompt }) => {
        console.log(`[CINEMATIC] Starting cinematic video generation for: ${prompt}`);
        return await generateVideo({
          prompt,
          style: "cinematic",
          narrationPrompt: `Write a poetic cinematic script (3-5 short lines) about: ${prompt}`,
          visualPrompt: `Moody, dramatic film-style visuals for: ${prompt}`,
        });
      },
    }),
  ],
...
app.post("/api/generate-video", async (req: any, res: any) => {
  ...
  try {
    ...
    console.log("[API] Agent completed successfully, returning file paths:", output);
    res.json(output);
  } catch (error) {
